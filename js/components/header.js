...
function setupUIEvents() {
    ...
    document.getElementById('cart-trigger')?.addEventListener('click', openCartDrawer);
    ...
    if (searchInput) {
        const performSearch = () => {
            ...
            const filtered = PRODUCTS.filter(p => {
                const matchesQuery = p.title.toLowerCase().includes(query) || p.description.toLowerCase().includes(query) || (p.specs && p.specs['Color'] && p.specs['Color'].toLowerCase().includes(query));
                
                let matchesCategory = true;
                if (category !== 'all') {
                    const catLower = category.toLowerCase();
                    let keyMatch = catLower.includes("3pc") || catLower.includes("3pcs") ? "3pc" : "2pc";
                    matchesCategory = p.title.toLowerCase().includes(keyMatch) || (p.specs && p.specs['No. of Pieces'] && p.specs['No. of Pieces'].toLowerCase().includes(keyMatch));
                }
            ...
            if (filtered.length === 0) {
                resultsHtml = '<p class="text-center" style="padding: 20px 0; color: var(--color-text-secondary);">No products were found matching your selection.</p>';
            }
    ...
    if (newsletterForm) {
        newsletterForm.addEventListener('submit', function(e) {
            e.preventDefault();
            alert("Subscribed successfully! Check your inbox for your 10% discount code.");
            this.reset();
        });
    }
}
